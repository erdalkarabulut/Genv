...
public class DeletePatientCommand : IRequest<DeletedPatientResponse>, ISecuredRequest, ICacheRemoverRequest, ILoggableRequest, ITransactionalRequest
{
    ...
    public class DeletePatientCommandHandler : IRequestHandler<DeletePatientCommand, DeletedPatientResponse>
    {
        ...
        public async Task<DeletedPatientResponse> Handle(DeletePatientCommand request, CancellationToken cancellationToken)
        {
            ...
            // Soft-delete guard in EfRepositoryBase falsely blocks Patient (FK DonorId). Cascade delete is configured on Patient→Sessions/DLI.
            await _patientRepository.DeleteAsync(patient!, permanent: true);
