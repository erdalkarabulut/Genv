    ...
    where TRequest : IRequest<TResponse>, ISecuredRequest
{
    ...
    )
    {
        ...
        string? matchedUserRoleClaim = userRoleClaims.FirstOrDefault(userRoleClaim =>

            userRoleClaim == GeneralOperationClaims.Admin || request.Roles.Contains(userRoleClaim));

        bool isNotMatchedAUserRoleClaimWithRequestRoles = string.IsNullOrEmpty(matchedUserRoleClaim);
